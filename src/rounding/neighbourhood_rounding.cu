...
/*
 * GPU-Accelerated Neighbourhood Rounding Kernel
 *
 * Generates K candidate solutions by:
 * 1. Rounding LP solution to nearest integers
 * 2. Using lock information to guide rounding direction
 * 3. Evaluating constraint violation for each candidate
 *
 * Thread organization:
 * - Each thread handles one candidate (index k)
 * - Threads explore different rounding choices for free variables
 */
...
__global__ void neighbourhood_rounding_kernel(
    ...
    // Lock information for intelligent rounding
    const int* locks_up,
...
/*
 * Main GPU rounding function
 *
 * Strategy:
 * 1. Compute lock information (CPU)
 * 2. Generate K candidates on GPU with varied rounding
 * 3. Sort candidates by violation score
 * 4. Attempt repair on best candidates
 */
Solution run_gpu_neighbourhood_rounding(
    ...
    // Sort candidates by violation score
    ...
    int max_repair_attempts = std::min(K, 20);
    for (int attempt = 0; attempt < max_repair_attempts; attempt++) {
        ...
        // If low violation, try without repair first
        if (scores[idx] < 1e-6) {
